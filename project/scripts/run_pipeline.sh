...
BUFFER_SIZE=6000
MAX_REQ=65536
TRACE_DIR="./traces_mix"
OUTPUT_CSV="results_mix.csv"
T="./t"
SIM="./simulator"
...
for i in $(seq 1 $NUM_WORKLOADS); do
    ...
    echo -n "trace_$i" > tmp_row_mix.csv
    ...
    echo ",$best_policy" >> tmp_row_mix.csv
    cat tmp_row_mix.csv >> $OUTPUT_CSV
    echo "[✔] 저장 완료: trace_$i → $best_policy ($best_rate%)"
done

rm -f tmp_row_mix.csv
