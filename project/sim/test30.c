...
typedef enum {
    FIFO,         // 0. 기본값
    LRU,          // 1. T1(전체버퍼)을 LRU 캐시로 사용, 참조용 LRU-ARC(p,t1,t2,b1,b2) 상태 업데이트
    LFU,          // 2. T3(전체버퍼)을 LFU 캐시로 사용, 참조용 LFU-ARC(q,t3,t4,b3,b4) 상태 업데이트
    LRU_ARC,      // 3. T1/T2 기반 원래 ARC
    LFU_ARC,      // 4. T3/T4 기반 원래 ARC
    CLOCK_T1,     // 5. T1을 사용하는 기본 CLOCK 정책
    CLOCK_T3,     // 6. T3을 사용하는 기본 CLOCK 정책
    CLOCK_PRO_T1_B4_LOGS_B2, // 7. T1캐시, B4히스토리, B2로그
    CLOCK_PRO_T3_B2_LOGS_B4  // 8. T3캐시, B2히스토리, B4로그
} ReplacementPolicy;
...
typedef struct {
    ...
    int list_type; // 현재 정책에 따른 실제 리스트 타입
                   ...
                   // LRU_ARC: 1(T1), 2(T2)
                   // LFU_ARC: 3(T3), 4(T4)
                   // CLOCK_T1, CLOCK_PRO_T1...: 1 (T1 캐시)
                   // CLOCK_T3, CLOCK_PRO_T3...: 3 (T3 캐시)
...
void initialize_buffer() {
    ...
    misses = 0;
    current_time = 0;
    previous_policy_for_state_carryover = FIFO;
    global_clk_hand = 0;
}
...
void initialize_arc_state(int full_reset) {
    if (full_reset) {
        ...
        // 정책별 특화 초기화
        if (current_policy == CLOCK_T1) {
            arc_state.p = (buffer_size > 0) ? buffer_size : 0;
            arc_state.t1_size = 0;
        } else if (current_policy == CLOCK_T3) {
            arc_state.q = (buffer_size > 0) ? buffer_size : 0;
            arc_state.t3_size = 0;
        } else if (current_policy == CLOCK_PRO_T1_B4_LOGS_B2) {
            arc_state.p = (buffer_size > 0) ? buffer_size / 2 : 0;
            arc_state.t1_size = 0;
        } else if (current_policy == CLOCK_PRO_T3_B2_LOGS_B4) {
...
void access_page(unsigned long long lba_address, unsigned long long page_id, int operation_type) {
    ...
    // ========================
        ...
        // --- ARC 파라미터 조정 및 로드될 리스트 결정 ---
        if (current_policy == LRU || current_policy == LRU_ARC) {
            ...
            if (current_policy == LRU) actual_load_list_type = 1;

        } else if (current_policy == LFU || current_policy == LFU_ARC) {
            ...
            if (current_policy == LFU) actual_load_list_type = 3;

        } else if (current_policy == CLOCK_T1) {
            actual_load_list_type = 1;
        } else if (current_policy == CLOCK_T3) {
            actual_load_list_type = 3;
        } else if (current_policy == CLOCK_PRO_T1_B4_LOGS_B2) {
        ...
        if (target_slot == -1) {
            ...
            // 정책별 희생자 선택
            if (current_policy == FIFO) victim_idx = evict_fifo();
            else if (current_policy == LRU) victim_idx = evict_arc_internal_lru(1);
            else if (current_policy == LFU) victim_idx = evict_arc_internal_lfu(3);
            else if (current_policy == LRU_ARC) victim_idx = arc_find_victim_lru_arc(page_id);
            else if (current_policy == LFU_ARC) victim_idx = arc_find_victim_lfu_arc(page_id);
            else if (current_policy == CLOCK_T1) victim_idx = evict_via_clock_policy(&global_clk_hand, 0, 0, buffer, buffer_size, policy_names[current_policy]);
            else if (current_policy == CLOCK_T3) victim_idx = evict_via_clock_policy(&global_clk_hand, 0, 0, buffer, buffer_size, policy_names[current_policy]);
            else if (current_policy == CLOCK_PRO_T1_B4_LOGS_B2) {
            ...
            if (victim_idx != -1) {
                ...
                // --- 정책별 고스트 리스트 및 로그/히스토리 업데이트 ---
                if (current_policy == LRU && evicted_page_id != INVALID_PAGE) {
                ...
                } else if (current_policy == LFU && evicted_page_id != INVALID_PAGE) {
                ...
                else if (current_policy == CLOCK_PRO_T1_B4_LOGS_B2 && evicted_page_id != INVALID_PAGE) {
                    arc_add_to_ghost_mru(evicted_page_id, arc_state.b4, &arc_state.b4_size, buffer_size);
                    arc_add_to_ghost_mru(evicted_page_id, arc_state.b2, &arc_state.b2_size, buffer_size);
                    if(arc_state.t1_size > 0 && buffer[victim_idx].list_type == 1) arc_state.t1_size--;
                } else if (current_policy == CLOCK_PRO_T3_B2_LOGS_B4 && evicted_page_id != INVALID_PAGE) {
                    arc_add_to_ghost_mru(evicted_page_id, arc_state.b2, &arc_state.b2_size, buffer_size);
                    arc_add_to_ghost_mru(evicted_page_id, arc_state.b4, &arc_state.b4_size, buffer_size);
        ...
        if (target_slot != -1) {
            ...
            buffer[target_slot].list_type = actual_load_list_type;
            buffer[target_slot].ref_bit = 1;

            // 정책별 리스트 크기 및 참조 상태 업데이트
            if (current_policy == LRU) {
            ...
            else if (current_policy == LRU_ARC) {
                ...
                if (actual_load_list_type == 3) arc_state.t3_size++; else if (actual_load_list_type == 4) arc_state.t4_size++;
                 ...
                 buffer[target_slot].ref_arc_list_type = 0;
                 buffer[target_slot].list_type = 1;
                 if (arc_state.t1_size < buffer_size) arc_state.t1_size++;
            } else if (current_policy == CLOCK_T3) {
                 buffer[target_slot].ref_arc_list_type = 0;
                 buffer[target_slot].list_type = 3;
                 if (arc_state.t3_size < buffer_size) arc_state.t3_size++;
            } else if (current_policy == CLOCK_PRO_T1_B4_LOGS_B2) {
                buffer[target_slot].ref_arc_list_type = 0;
                buffer[target_slot].list_type = 1;
                if (arc_state.t1_size < buffer_size) arc_state.t1_size++;
                arc_add_to_ghost_mru(page_id, arc_state.b2, &arc_state.b2_size, buffer_size);
            } else if (current_policy == CLOCK_PRO_T3_B2_LOGS_B4) {
                buffer[target_slot].ref_arc_list_type = 0;
                buffer[target_slot].list_type = 3;
                if (arc_state.t3_size < buffer_size) arc_state.t3_size++;
                arc_add_to_ghost_mru(page_id, arc_state.b4, &arc_state.b4_size, buffer_size);
            } else { // FIFO
                 buffer[target_slot].ref_arc_list_type = 0;
                 buffer[target_slot].ref_bit = 0;
                 buffer[target_slot].list_type = 0;
            }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        ...
        fprintf(stderr, "사용 가능 정책: FIFO, LRU, LFU, LRU_ARC, LFU_ARC, CLOCK_T1, CLOCK_T3, CLOCK_PRO_T1_B4_LOGS_B2, CLOCK_PRO_T3_B2_LOGS_B4\n");
    ...
    // 정책 이름 비교 및 설정
    if (strcmp(initial_policy_arg_lower, "lru") == 0) current_policy = LRU;
    else if (strcmp(initial_policy_arg_lower, "lfu") == 0) current_policy = LFU;
    else if (strcmp(initial_policy_arg_lower, "lru_arc") == 0) current_policy = LRU_ARC;
    else if (strcmp(initial_policy_arg_lower, "lfu_arc") == 0) current_policy = LFU_ARC;
    else if (strcmp(initial_policy_arg_lower, "clock_t1") == 0) current_policy = CLOCK_T1;
    else if (strcmp(initial_policy_arg_lower, "clock_t3") == 0) current_policy = CLOCK_T3;
    else if (strcmp(initial_policy_arg_lower, "clock_pro_t1_b4_logs_b2") == 0) current_policy = CLOCK_PRO_T1_B4_LOGS_B2;
    else if (strcmp(initial_policy_arg_lower, "clock_pro_t3_b2_logs_b4") == 0) current_policy = CLOCK_PRO_T3_B2_LOGS_B4;
    else if (strcmp(initial_policy_arg_lower, "fifo") == 0) current_policy = FIFO;
    else {
        ...
        *(end_of_line + 1) = '\0';
        ...
        if (toupper(trimmed_line[0]) == 'P' && (trimmed_line[1] == ' ' || trimmed_line[1] == '\t')) {
            ...
            if (sscanf(trimmed_line, "%c %d", &cmd_char, &new_policy_code) == 2 && toupper(cmd_char) == 'P') {
                if (new_policy_code >= FIFO && new_policy_code <= CLOCK_PRO_T3_B2_LOGS_B4) {
                    ...
                    if (old_policy != new_policy) {
                        ...
                        initialize_arc_state(reset_arc_completely);
                        ...
                        for (int i = 0; i < buffer_size; i++) {
                            if (buffer[i].page_id != INVALID_PAGE) {
                                ...
                                } else if (new_policy == CLOCK_T1 || new_policy == CLOCK_PRO_T1_B4_LOGS_B2) {
                                    buffer[i].list_type = 1;
                                    arc_state.t1_size++;
                                    buffer[i].ref_bit = 1;
                                } else if (new_policy == CLOCK_T3 || new_policy == CLOCK_PRO_T3_B2_LOGS_B4) {
                                    buffer[i].list_type = 3;
                                    arc_state.t3_size++;
                                    buffer[i].ref_bit = 1;
                                } else { // FIFO
                ...
                } else { fprintf(stderr, "경고: (라인 %d) 잘못된 정책 코드 %d. 무시.\n", line_num, new_policy_code); }
    ...
    int num_policies = sizeof(policy_names) / sizeof(policy_names[0]);
    ...
    if (!initial_policy_was_valid) {
        summary_initial_policy_name_to_print = policy_names[FIFO];
    }
